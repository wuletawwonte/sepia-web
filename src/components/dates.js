...
export function FullTime(props) {
  if (props.date) {
    ...
    return dateObj.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12 });
  }
  return '';
}
...
export function SelectMonthByName(props) {
  const monthNames = (props.shortForm) ? MonthInNames : MonthInFullNames;
...
export function DateInput(props) {
  ...
  return (
    <input
      ...
      name={props.name}
      aria-label={props.label}
      className={useExtendClass('form-control', props.className)}
      value={props.value}
