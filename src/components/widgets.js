...
import {
  Dropdown, DropdownButton, DropdownMenu, DropdownMenuButton,
} from './dropdown';
...
export function WidgetDeck(props) {
  return (
    <div className={useExtendClass('card-deck mb-3', props.className)}>
      {props.children}
    </div>
  );
}
...
export function WidgetColumns(props) {
  return (
    <div className={useExtendClass('card-columns mb-3', props.className)}>
      {props.children}
    </div>
  );
}
