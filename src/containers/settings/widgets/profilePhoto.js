...
import {
  Form, FormButton, FormGroup, FormLabel, FormRow, FormSubmit,
} from '../../../components/form';
import { Col, FluidContainer, Row } from '../../../components/layout';
import { Photo } from '../../../components/users';
import {
  TitleBar, TitleButton, Widget, WidgetBody,
...
function EditSection(props) {
  ...
  async function clickedDelete(e) {
    ...
    try {
      const { username } = props.session;
      const { profilePhotoId } = props.session;
      ...
      <FormRow>
        ...
        <FormGroup className="col-12 col-sm-auto">
          <FormRow>
            ...
            />
            <label className="custom-file-label" htmlFor="generalInput07">
              {
                                (fields.name)
                                  ? fields.name.replace(/^.*[\\\/]/, '')
                                  : 'Choose a file...'
