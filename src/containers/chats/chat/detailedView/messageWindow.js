...
export default function MessageWindow(props) {
  ...
  for (const [idx, message] of state.messages.entries()) {
    ...
    const currentUsername = message.sender;
    const isCurrentUser = (props.session.username === currentUsername);

    const chatAlign = (isCurrentUser) ? 'justify-content-end' : 'justify-content-start';
    const chatColor = (isCurrentUser) ? 'md-chat-out' : 'md-chat-in';
    const chatSpacing = (lastUsername === currentUsername) ? 'mt-1' : 'mt-3';
  ...
  return (
    <Row className="md-msg-window flex-grow-1 overflow-y border-top border-bottom">
      <Col>
        ...
        {messageItems}
        <span ref={bottomRef} className="p-1" />
      </Col>
    </Row>
  );
}
